...
if fetch_button:
    ...
    with st.spinner(f"正在抓取 {stock_name} 的历年财务大表..."):
        try:
            ...
            for en_col, cn_col in metric_mapping.items():
                if en_col in annual_df.columns:
                    core_data[cn_col] = annual_df[en_col]
            
            if 'PARENT_NETPROFIT' in annual_df.columns:
                core_data['归母净利润'] = annual_df['PARENT_NETPROFIT']
            elif 'NETPROFIT' in annual_df.columns:
                core_data['净利润'] = annual_df['NETPROFIT']
